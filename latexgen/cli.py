...
def main():
    ...
    preamb = questionary.select(
        msg["select_preamble"], choices=list(PREAMBLE_OPTIONS.keys())
    ).ask()
    fonte = questionary.select(
        msg["select_font"], choices=list(FONT_OPTIONS.keys())
    ).ask()
    ...
    data = questionary.text(
        msg["enter_date"], default=datetime.today().strftime("%Y-%m-%d")
    ).ask()
    ...
    if PREAMBLE_OPTIONS[preamb] == "custom":
        ...
        selected_packages = questionary.checkbox(
            msg["select_packages"], choices=pkgs
        ).ask()
